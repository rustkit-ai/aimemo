#!/usr/bin/env bash
# Updates Formula/aimemo.rb with correct SHA256 hashes from a GitHub release.
...
for target in "${targets[@]}"; do
  ...
  sed -i.bak "/aimemo-${target}\.tar\.gz/{n;s/sha256 .*/sha256 \"${sha}\"/}" "$FORMULA"
done
